...
import { cpSync, readFileSync, readdirSync, renameSync, rmSync, writeFileSync } from 'fs';
import { join } from 'path';
import { InferredOptions, SafetyNetBuildException, log, rollbackLine, success } from '../utils/utils';
...
export const generateModels = async (opts: InferredOptions): Promise<void> => {
    try {
        ...
        const s = spawnSync(`npx`, [`openapi-generator-cli`, `generate`,
            ...
            {});
        if (s.error) {
            throw new SafetyNetBuildException(exceptions.failedToGenerateModelsUsingOpenAPIGenerator, s.error);
        }
        cpSync(join(opts.paths.tmp, tmpModelPackageDir), `${opts.paths.models}`, { recursive: true });
        ...
        for (const modelFilename of readdirSync(`${opts.paths.models}`)) {
            ...
            if (modelName != modelNameCamelCase) {
                renameSync(join(opts.paths.models, modelFilename), join(opts.paths.models, `${modelNameCamelCase}${ext}`));
            }
        }
